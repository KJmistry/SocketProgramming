...
int main(void)
{
    ...
    if (setsockopt(clientFd, SOL_SOCKET, SO_RCVTIMEO, (struct timeval *)&tv, sizeof(struct timeval)))
    {
        printf("setsockopt : %s\n",strerror(errno));
        exit(EXIT_FAILURE);
    }
    ...
    while (1)
    {
        ...
        case 2:
            ...
            snprintf(finalData, FINAL_DATA_BUFFER_SIZE, "2%c%d%c", specialCharacter, deleteRollNum, null);
            printf("Final Frame Generated : %s\n", finalData);
            break;
